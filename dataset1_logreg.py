...
class DataSet:
    ...
    __x1_mean   = np.array([2,2])
    __x0_cov    = np.array([[1,-0.5], [-0.5,1]])
    __x1_cov    = np.array([[1,-0.5], [-0.5,1]])
    __x_D       = None  
    __y_D       = None
    ...
    def get_data(cls):
        if cls.__y_D is None and cls.__x_D is None:
            ...
            x1 = np.random.multivariate_normal(cls.__x1_mean, cls.__x1_cov, cls.__N_samples).T
